...
if (strlen($_POST['directory']) > 16) {
    Finish('ディレクトリ名は16文字以下で記入して下さい');
}
if (file_exists('../' . $_POST['directory'])) {
    Finish('ディレクトリ名は既に使用されています');
}
if (preg_match('/[^a-z0-9]/', $_POST['directory'])) {
    Finish('ディレクトリ名には半角英数小文字のみ使用できます');
}
