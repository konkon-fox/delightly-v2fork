...
foreach ($dirList as $path) {
    ...
    $logFile = $path . '/errors.cgi';
    $bbsList[] = [
				'bbs' => $dir,
				'migrated' => !is_file($logFile),
		];
}
...
if ($_POST['edit'] === 'true' && isset($_POST['bbs'])) {
    ...
    $db = new BbsDB($bbsPath);
    $result = $db->migrateErrorLog();
}
...
<head>
	...
	<title>エラーログをDB化(v2,v3 => v4)</title>
	<link
...
<body>
    ...
    <header class="d-flex flex-column row-gap-1">
		...
		<h1>エラーログをDB化(v2,v3 => v4)</h1>
		<main class="d-flex flex-column row-gap-2">
			<?php
                    foreach ($bbsList as $bbsData) {
                        ?>
							<div class="card">
								<form action="?mode=migration-error-log" method="post">
									<input
										type="hidden"
										name="code"
  								...
  								<div class="card-body">
										<p class="card-text">
											...
											if ($bbsData['migrated']) {
											?>
												<button class="btn btn-secondary" disabled>移行不要・移行済み</button>
											<?php
											} else {
											?>
