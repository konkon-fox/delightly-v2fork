...
foreach ($dirList as $path) {
    ...
    $subjectFile = $path . '/kakolog-subject.txt';
    $bbsList[] = [
				'bbs' => $dir,
				'migrated' => !is_file($subjectFile),
		];
}
...
<head>
	...
	<title>過去ログ一覧ファイルをDB化(v3 => v4)</title>
	<link
...
<body>
    ...
    <header class="d-flex flex-column row-gap-1">
		...
		<h1>過去ログ一覧ファイルをDB化(v3 => v4)</h1>
		<main class="d-flex flex-column row-gap-2">
			<?php
					foreach ($bbsList as $bbsData) {
							?>
							<div class="card">
								<form action="?mode=migration3to4" method="post">
									<input
										type="hidden"
										name="code"
