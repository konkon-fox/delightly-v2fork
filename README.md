...
本スクリプトは認証時に `proxycheck.io` の API を利用しています。  
[利用規約](https://proxycheck.io/terms/)をご確認の上、ご使用ください。
...
1日の利用制限（クエリ上限）を超えると、APIはエラーを返します。  
この場合、認証ページではセキュリティを優先し、 **認証を一時的に停止（エラーメッセージを表示）** する仕様となっています。  
アクセス数が多いサイトで運用する場合は、無料プランへの登録、または有料プランの検討を推奨します。
