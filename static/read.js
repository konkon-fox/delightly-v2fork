...
if (autodark == 'true' && window.matchMedia('(prefers-color-scheme: dark)').matches === true)
  darkmode = 'true';
let mutejson;
let mutelist = [];
...
if (localStorage.getItem('formfixed') == 'true') {
  ...
  document.getElementById('postForm').innerHTML +=
    '<label><input type="checkbox" id="isFixedForm" onchange="FixedForm();" checked>入力フォーム位置固定</label>';
} else
  document.getElementById('postForm').innerHTML +=
    '<label><input type="checkbox" id="isFixedForm" onchange="FixedForm();">入力フォーム位置固定</label>';
let mutes = '';
function load(mode) {
  //スレッド描画
  ...
  request.onreadystatechange = () => {
    if (request.readyState === 4 && request.status === 200) {
      ...
      data.forEach(function (value) {
        ...
        if (target) {
          if (
            message.search(target) == -1 &&
            name.search(target) == -1 &&
            mail.search(target) == -1 &&
            dateid.search(target) == -1
          )
            NG = true;
        }
        if (dat[4]) {
        ...
        if (document.getElementById('back-' + rnum)) {
          if (treeview != 'true')
            document.getElementById('back-' + rnum).innerHTML +=
              '<a class="back-link" href="javascript:void(0);">&gt;&gt;' + number + '</a>';
          ...
          if (replycount < 3) document.getElementById('number-' + rnum).style.color = '#518dc9';
            ...
            document.getElementById('number-' + rnum).style.color = '#a551c9';
          else if (replycount < 10)
            document.getElementById('number-' + rnum).style.color = 'darkred';
        ...
        if (treeview == 'true' && rnum && document.getElementById(rnum)) {
          let tleft = 10;
          if (document.getElementById(rnum).dataset.tree)
            tleft = document.getElementById(rnum).dataset.tree * 1.5;
          newpost.dataset.tree = tleft;
          if (darkmode == 'true')
            newpost.style.cssText = 'border-left:.5px solid #333;margin-left:' + tleft + 'px';
        ...
        if (isSmartPhone() == true)
          newpost.innerHTML =
            ...
            number +
            '" class="rcount"></small><span class="backs-front"> :</span><span id="back-' +
            number +
            '"></span></div>';
        else
          newpost.innerHTML =
        ...
        if (!loaded && number == localStorage.getItem(bbs + key)) {
          ...
          if (darkmode == 'true')
            readline.style.cssText =
              'background-color: #333333;color: #fff;text-align: center;font-size: 80%;';
          else
            readline.style.cssText =
              'background-color: #f0f0f0;color: #666666;text-align: center;font-size: 80%;';
          document.getElementById(number).after(readline);
        }
        LINENUM = number;
        document.getElementById('count').innerHTML = LINENUM + 'レス';
        if (areload != false && number > localStorage.getItem(bbs + key))
  ...
  if (getCookie('response')) {
    if (decodeURI(getCookie('response')) != 'success') {
      ...
      if (sessionStorage.getItem('text'))
        document.getElementById('bbs-textarea').value = sessionStorage.getItem('text');
    } else {
      sessionStorage.removeItem('text');
...
if (!getCookie('WrtAgreementKey'))
  document.getElementById('postForm').innerHTML =
    '<div><a href="/test/auth.php">投稿時に使用する同意鍵がありません。<br>投稿を行うには投稿前確認画面での同意が必要です。</a></div>' +
    document.getElementById('postForm').innerHTML;
if (!getCookie('icon') && document.getElementById('seticon'))
...
if (getprev) {
  ...
  prevbutton.innerHTML =
    '<a name="1"></a><a href="#1" onclick="prev(' + getprev + ')">前のレスを取得</a>';
...
modal.id = 'rModal';
modal.style.cssText =
  'position: fixed; z-index: 20; left: 0px; top: 0px; width: 100%; height: 100%; overflow: auto; background-color: rgba(0, 0, 0, 0.4);';
modal.style.display = 'none';
document.getElementById('body').appendChild(modal);

let notific = document.createElement('div');
notific.innerHTML =
  '<div id="ntxt" style="font-size:13px;background-color:#404040;width:18%;margin:0 auto;text-align:center;border-radius:3px;padding:4px">' +
  '</div>';
notific.id = 'notific';
notific.style.cssText =
  'display:none;opacity:0;bottom:5%;color:#fff;position:fixed;width:100%;z-index:20';
...
function Menu(n) {
  document.getElementById('modal_text').innerHTML =
    '<a class="menulink" href="javascript:Num(\'\')">投稿欄を開く</a>&emsp;<a class="menulink" href="javascript:Num(\'' +
    n +
    '\')">&gt;&gt;' +
    n +
    ' へ返信</a><div style="font-weight:bold;">ミュート設定</div><a class="menulink" href="javascript:mute(\'list\')">ID</a>&emsp;<a class="menulink" href="javascript:MenuClick(\'ngword\')">Word</a>&emsp;<a class="menulink" href="javascript:MenuClick(\'ngtitle\')">タイトル</a><div style="font-weight:bold;">レス情報コピー</div><a class="menulink" href="javascript:MenuClick(\'Copyr-' +
    n +
    '\')">レス</a>&emsp;<a class="menulink" href="javascript:MenuClick(\'Copyur-' +
    n +
    '\')">URL+レス</a>&emsp;<a class="menulink" href="javascript:MenuClick(\'Copytur-' +
    n +
    '\')">タイトル+URL+レス</a>&emsp;<a class="menulink" href="javascript:MenuClick(\'Copyu-' +
    n +
    '\')">URL</a><div style="font-weight:bold;">スレッド</div><a class="menulink" href="javascript:logclear()">このスレのログを削除</a>&emsp;<a <a class="menulink" href="/test/createthread.php?bbs=' +
    bbs +
    '&key=' +
    key +
    '">次スレッド作成</a><br><a class="menulink" href="javascript:MenuClick(\'CopyThreadLink\')">タイトルとスレッドURLをコピー</a><br><a class="menulink" href="javascript:MenuClick(\'CopyLink\')">スレッドURLをコピー</a><br><div style="font-weight:bold;">その他</div><a class="menulink" href="javascript:MenuClick(\'setting\')">閲覧設定</a>&emsp;<a class="menulink" href="/test/backimg.html">背景画像</a>&emsp;<a class="menulink" href="/test/css.html">カスタムcss</a><br><a class="menulink" href="javascript:setclear()">全ログを削除</a>&emsp;<a class="menulink" href="/test/auth.php">認証</a>';
...
function ResAnchor(n, r) {
  ...
  if (!document.getElementById('reply-' + r + '-' + n))
    document.getElementById('msg-' + r).innerHTML =
      '<dl class="rep-comment" id="reply-' +
      r +
      '-' +
      n +
      '">' +
...
function ResAnchor2(a, b) {
  ...
  for (let c = a; c < d; c++) {
    ...
    if (document.getElementById(c))
      document.getElementById('modal_text').innerHTML += document.getElementById(c).innerHTML;
  }
  document.getElementById('rModal').style.display = 'block';
}
...
function IdClick(ID, n) {
  ...
  let ins =
    ...
    '</span> ' +
    idcount +
    'レス</div><div><a class="mbutton" href="javascript:mute(\'' +
    ID +
    '\')" style="color:black;">ミュートする</a></div>';
...
function ID(d, n, num) {
  ...
  if (idcount == 1)
    id = '<a class="id" href="javascript:IdClick(\'' + did + "'," + num + ')">' + did + '</a>';
  else if (idcount < 3)
    id =
      '<a class="id" href="javascript:IdClick(\'' +
      did +
      "'," +
      num +
      ')" style="color: #518dc9 !important;">' +
      did +
      '(' +
      idcount +
      ')</a>';
...
function mute(target) {
  if (target == 'list') {
    let mlisthtml =
      '<div style="font-weight:bold;">ミュートするID</div><input type="text" id="addmute"><button onclick="AddM()">追加</button>&emsp;<button onclick="mute(\'reset\')">リセット</button><br>※クリックで解除<br>';
    for (let i = 0; i < mutelist.length; i++) {
      mlisthtml +=
        '<a class="menulink" href="javascript:unmute(\'' +
        mutelist[i] +
        '\')">' +
        mutelist[i] +
        '</a>&emsp;';
...
function MenuClick(val) {
  if (val.indexOf('Copyr-') != -1) {
    ...
    if (navigator.clipboard) {
      ...
      let text =
        n +
        ' ' +
        document.getElementById('name-' + n).innerText +
        ' ' +
        d +
        '\n' +
  ...
  if (val == 'setting') {
    ...
    document.getElementById('modal_text').innerHTML =
      '<div class="option_style_2">閲覧設定</div><div class="option_style_3">&emsp;</div><div class="option_style_4"><div class="option_style_5"><input class="option_style_6" ' +
      spcheck +
      ' id="spmode" type="checkbox">スマホ用表示</div><div class="option_style_5"><input class="option_style_6" ' +
      darkcheck +
      ' id="darkmode" type="checkbox">ダークモード</div><div class="option_style_5"><input class="option_style_6" ' +
      adarkcheck +
      ' id="autodark" type="checkbox">デバイスのダークモードと同期する</div><div class="option_style_5"><input class="option_style_6" ' +
      arecheck +
      ' id="arecheck" type="checkbox">5秒間隔で自動更新する</div><div class="option_style_5"><input class="option_style_6" ' +
      autocheck +
      ' id="autoscroll" type="checkbox">新着投稿の位置まで自動スクロール</div><div class="option_style_5"><input class="option_style_6" ' +
      gurocheck +
      ' id="g_hide" type="checkbox">注意のある画像を自動的に非表示</div><div class="option_style_5"><input class="option_style_6" ' +
      gcheck +
      ' id="a_hide" type="checkbox">画像のサムネイル表示をオフにする</div><div class="option_style_5"><input class="option_style_6" ' +
      treecheck +
      ' id="tview" type="checkbox">ツリー表示</div><div class="option_style_5"><input class="option_style_6" ' +
      dcheck +
      ' id="origdate" type="checkbox">投稿日時表記の短縮を行わない</div></div><div class="option_style_11"><button id="saveOptions" class="option_style_12" onclick="setoption()">変更を保存</button><button id="cancelOptions" class="option_style_13" onclick="rclose()">キャンセル</button></div>';
    document.getElementById('rModal').style.display = 'block';
    return;
  }
  if (val == 'ngword') {
    ...
    nlisthtml +=
      '<input type="text" id="addngword"><button onclick="NGWORD()">追加</button>&emsp;<button onclick="unngword(\'reset\')">リセット</button><br>';
    for (let i = 0; i < nglist.length; i++) {
      nlisthtml +=
        '<a class="menulink" href="javascript:unngword(\'' +
        nglist[i] +
        '\')">' +
        nglist[i] +
        '</a>&emsp;';
  ...
  if (val == 'ngtitle') {
    ...
    nlisthtml +=
      '<input type="text" id="addngtitle"><button onclick="NGTITLE()">追加</button>&emsp;<button onclick="unngtitle(\'reset\')">リセット</button><br>';
    for (let i = 0; i < ntlist.length; i++) {
      nlisthtml +=
        '<a class="menulink" href="javascript:unngtitle(\'' +
        ntlist[i] +
        '\')">' +
        ntlist[i] +
        '</a>&emsp;';
