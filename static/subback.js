...
document.head.innerHTML +=
  '<link href="/static/st.css" rel="stylesheet" type="text/css"><link href="/static/milligram.css" rel="stylesheet" type="text/css"><link href="/static/s.css" rel="stylesheet" type="text/css"><link href="/static/index.css" rel="stylesheet" type="text/css"><link href="/static/read.css" rel="stylesheet" type="text/css">';
document.body.innerHTML =
  '<section class="section"><div id="body"><div id="followheader" class="hidden maxwidth100 height2p5 stickymenu container" style="display: block;"><div class="row noflex maxwidth100 white padding0p5 maxheight2p5 borderbottomlightgrey"><div class="topmenu" id="header"><a class="menuitem" href="/' +
  bbs +
  ...
  bbs +
  '">スレ作成</a><a class="menuitem" href="javascript:Menu()">メニュー</a><a href="javascript:MenuClick(\'setting\')" class="menuitem">設定</a><div id="headtitle" class="menuitem" style="text-decoration: none;"></div></div></div></div><div id="subbacktitle"></div><div class="tlnotice" id="ntxt" style="margin-top:3rem"></div><div class="search"><input type="text" id="searchInput" placeholder="スレッドタイトル検索" onchange="search()"></div><div class="newposts"><center><a href="javascript:load()">リロード<img alt="再読み込み" loading="lazy" decoding="async" data-nimg="1" style="color: transparent;" src="/static/reload.svg" width="16" height="16"></a></center><hr></div><div id="main"></div><div id="hidemetoo" class="side p85 slightpad nobullets"><div style="min-height:50px" id="rule">■ ローカルルール<br></div><div style="min-height:50px" id="kokuti">■ 告知欄<br></div><p style="margin:5rem 0;color:#333"><a href="https://git.3chan.cc/stat2/delightly-v2fork">スレッドフロート型掲示板「delightly」v2 fork</a> , <a href="https://delightly.xyz/">nore</a> +</p></div></div></section>';
...
if (isSmartPhone() == false)
  document.getElementById('body').innerHTML +=
    '<style>a{color:#485269 !important;}#headtitle,b{color:rgb(87, 111, 118) !important}</style>';
else document.getElementById('body').innerHTML += '<style>.title{margin-top:.5em;}</style>';
document.getElementById('body').innerHTML +=
  '<style>div.postbutton{display: none !important;}#main{border-top:.5px solid #DCDCDC}</style>';
  ...
  request.onload = function () {
    if (request.response) {
      ...
      listJSON.forEach(function (post) {
        ...
        if (
          ls.get('mode') != 'archive' &&
          localStorage.getItem(bbs + key) &&
          number > localStorage.getItem(bbs + key)
        ) {
        ...
        if (isSmartPhone() == false)
          newpost.innerHTML =
            ...
            number +
            nres +
            ')</small><span class="date"><span class="created">' +
            created +
          ...
          newpost.innerHTML =
            ...
            '">' +
            title +
            '</div><span class="date"><span class="right"><span class="speed">' +
            t +
    ...
    if (ls.get('mode') != 'archive')
      document.getElementById('main').innerHTML +=
        '<div class="m threads"><b><a href="/' +
        bbs +
        '/?m=subback&mode=archive">過去ログ倉庫はこちら</a></b></div>';
    else {
      archive = kakolog - 1;
      document.getElementById('main').innerHTML +=
        '<div class="m threads"><b><a href="/' +
        bbs +
        '/?m=subback&mode=archive&archive=' +
        archive +
        '">次へ(' +
        archive +
        ')</a></b></div>';
...
function Menu() {
  document.getElementById('modal_text').innerHTML =
    '<div style="font-weight:bold;">ミュート設定</div><a class="menulink" href="javascript:mute(\'list\')">ID</a>&emsp;<a class="menulink" href="javascript:MenuClick(\'ngword\')">Word</a>&emsp;<a class="menulink" href="javascript:MenuClick(\'ngtitle\')">タイトル</a><div style="font-weight:bold;">その他</div><a class="menulink" href="javascript:MenuClick(\'setting\')">閲覧設定</a>&emsp;<a class="menulink" href="/test/backimg.html">背景画像</a>&emsp;<a class="menulink" href="/test/css.html">カスタムcss</a><br><a class="menulink" href="javascript:setclear()">全ログを削除</a>&emsp;<a class="menulink" href="/test/auth.php">認証</a>';
  document.getElementById('rModal').style.display = 'block';
}
...
function MenuClick(val) {
  ...
  if (val == 'setting') {
    ...
    document.getElementById('modal_text').innerHTML =
      ...
      gcheck +
      ' id="a_hide" type="checkbox">画像のサムネイル表示をオフにする</div></div><div class="option_style_11"><button id="saveOptions" class="option_style_12" onclick="setoption()">変更を保存</button><button id="cancelOptions" class="option_style_13" onclick="rclose()">キャンセル</button></div>';
    document.getElementById('rModal').style.display = 'block';
    return;
  }
}
